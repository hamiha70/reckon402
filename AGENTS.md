...
- Canonical narrative discipline on user-facing surfaces (READMEs, KH workflow descriptions, demo copy, submission drafts, runbook intros, landing page, npm package READMEs): use "trust signal" (NOT "trust score"), "history-aware behavior" (NOT "use cases"), per-settlement facilitator-signed ERC-8004 attestation, per-agent on-chain Escrow with pluggable `ITierStrategy` releasing as reputation grows. The dead pre-pivot vocabulary is permanently banned: "tier-based discount pricing", "5%/10%/15% off", price-contrast `100000 → 90000`, "extra 11% risk fee", and "discount" framing in any form. Defer to `docs/canonical-narrative.md` and grep for stale phrases before shipping.
- Cloudflare account safety: deploys for this repo MUST target the personal `Hamiha70@gmail.com` Cloudflare account (id `0f38f8667bbcbe4f54eda13c8df009e0`), NEVER the `intentralabs` Cloudflare org. Before any `wrangler pages deploy` / `wrangler deploy`, confirm the active `CLOUDFLARE_API_TOKEN` resolves to the correct account; cross-account misrouting is a hard incident — abort the deploy, do not retry under a different token.
- Solo-author hackathon submission: a single author is the only contributor; do not invent collaborators, teams, or co-authors on submission, landing, KH workflow, or any external-facing surface.
- On stale-narrative or hallucination drift, reset rather than patch in-context: if reasoning is anchored on a superseded layer (e.g. L4c price-discount story while L4d Escrow is locked) or invents sponsors/tracks/integrations, stop, identify the last clean commit/tag, and resume from a fresh tightly-scoped prompt citing `docs/canonical-narrative.md` plus the relevant AGENTS.md L4d sections — do not argue in-flight reasoning back to correctness. Symmetrically, when patching a NEW layer (e.g. L4d), preserve the LEGACY layer's scripts, recipes, and on-chain artefacts (e.g. `full-flow-l3.sh`, `just onboard`, `seller.reckon402-test.eth`, legacy splitter `0x372c0b95…`) — never silently regress a legacy path to make the new patch work; legacy and new must both stay green at every layer transition.
- Never run `git commit` / `git push` / `wrangler deploy` / `wrangler pages deploy` / `kh workflow {create,go-live}` (or any other production-mutating CLI) without an explicit "deploy" / "commit" / "push" / "publish" / "go" in the current operator turn; draft the exact command + commit message text for the operator to paste, but the operator executes it themselves so the commit graph, deploy log, and KH publish history reflect only operator intent. This is reiterated across every visual-iteration and infrastructure session — treat it as load-bearing, not optional politeness.
...
- Canonicality hierarchy for cross-surface consistency (when surfaces disagree, the higher entry wins): (1) `docs/canonical-narrative.md` — locked A0 source-of-truth for messaging (committed at `260fb5c`); (2) `AGENTS.md` — canonical technical facts (addresses, EOAs, KMS aliases, layer status, sponsor list); (3) `docs/trust-architecture.md` — trust-signal / Escrow / tier model narrative; (4) `docs/test-posture-h-9.md` — test counts and smoke-probe inventory; (5) live HTTP responses from deployed Workers (`reckon402.com/healthz`, `signing.reckon402.com/healthz`, etc.). Pre-submission consistency passes use this order to decide which surface is wrong.
- Cloudflare Workers' per-invocation subrequest cap applies on the Paid plan as well (1000 subrequests per request; free plan is 50). The L4d onboarding orchestrator (`workers/onboard-orchestrator`) has tripped this with chained `waitForTransactionReceipt` polling across the 6-step onboarding flow, surfacing as `Too many subrequests by single Worker invocation` from viem with a confusing dashboard-link error pointer. Mitigations when extending the orchestrator (or any long-running Worker handler that touches RPC): replace polling loops with single-shot `getTransactionReceipt` reads, batch RPC calls per step, or split the orchestration across multiple Worker invocations (Workflows, successive `fetch` chains, or `ctx.waitUntil` continuation in fresh requests). Confirming the account is on the Paid plan does NOT raise the limit further — the cap is per-invocation, not per-account.
