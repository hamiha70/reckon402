...
- On stale-narrative or hallucination drift, reset rather than patch in-context: if reasoning is anchored on a superseded layer (e.g. L4c price-discount story while L4d Escrow is locked) or invents sponsors/tracks/integrations, stop, identify the last clean commit/tag, and resume from a fresh tightly-scoped prompt citing `docs/canonical-narrative.md` plus the relevant AGENTS.md L4d sections — do not argue in-flight reasoning back to correctness.
